...
const creekWaPreset = (t: "inquiry" | "details") => `/r/creek-wa?t=${t}`;
...
interface CreekLeadPopupProps {
  open: boolean;
  onClose: () => void;
}

export function CreekLeadPopup({ open, onClose }: CreekLeadPopupProps) {
  useEffect(() => {
    if (!open) return;
  ...
  return createPortal(
    ...
    >
      <div
        className="cv-lead-popup-root lead-card"
        role="dialog"
        aria-modal="true"
        aria-labelledby="creek-lead-popup-title"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="cv-lead-popup-progress" aria-hidden>
          <span className="cv-lead-popup-progress-fill" />
        </div>

        <button
          type="button"
          className="cv-lead-popup-close"
          aria-label="إغلاق"
          onClick={onClose}
        >
        ...
        <div className="cv-lead-popup-copy">
          <div className="eyebrow-ar">كريك ڤيو · ماونتن ڤيو</div>
          <h2 className="lead-popup-title" id="creek-lead-popup-title">
            محتاج تفاصيل وأسعار؟
          </h2>
          <p className="lead-popup-lede">
            اتصل أو ابعت واتساب، أو سيب رقمك وهنبعتلك جدول الأسعار والماستر بلان.
          </p>
          <div className="cv-popup-cta-grid">
            ...
            <a className="btn btn-wa" href={creekWaPreset("inquiry")}>
              <WhatsAppIcon />
              واتساب
            </a>
          </div>
        ...
        <div className="cv-popup-divider">أو اترك رقمك</div>
